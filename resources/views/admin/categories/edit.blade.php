...
@section('content')
    ...
    <div class="bg-white p-4 rounded-md shadow-md flex justify-center items-center">
        <div class="flex-1 max-w-3xl p-8 m-4 shadow-md bg-white rounded-lg">
            <h4 class="text-2xl font-semibold text-blue-gray-800 mb-4 text-center">Edit New Category</h4>
            <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
                @method('PUT')
                @csrf
                <div class="mb-4">
                    <label for="priority" class="block text-sm font-medium text-gray-700">Priority</label>
                    <input type="number" name="priority" id="priority" value="{{ $category->priority }}" required
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">{{ $category->description }} </textarea>
                </div>
                ...
                @enderror
                <button type="submit"
                    class="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-md">Update
                    Category</button>
            </form>
        </div>
    </div>
