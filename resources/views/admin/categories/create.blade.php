...
@section('content')
    ...
    <div class="bg-white p-4 rounded-md shadow-md flex justify-center items-center">
        <div class="flex-1 max-w-3xl p-8 m-4 shadow-md bg-white rounded-lg">
            <h4 class="text-2xl font-semibold text-blue-gray-800 mb-4 text-center">Add New Category</h4>
            <form action="{{ route('admin.categories.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label for="priority" class="block text-sm font-medium text-gray-700">Priority</label>
                    <input type="number" name="priority" id="priority" value="{{ old('priority') }}" required
