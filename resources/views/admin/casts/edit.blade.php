...
@section('content')
    <div class="bg-white p-4 rounded-md shadow-md mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-gray-800">Edit Cast</h2>
    ...
    <div class="bg-white p-6 rounded-md shadow-md flex justify-center items-center">
        <div class="flex-1 max-w-3xl bg-gray-50 p-8 rounded-lg shadow-lg">
            <h4 class="text-3xl font-bold text-gray-900 mb-6 text-center">Edti Cast</h4>
            <form action="{{ route('admin.casts.update', $cast->id) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
                @csrf

                <div class="mb-6">
                    <label for="name" class="block text-sm font-medium text-gray-700">Name</label>
                    <input type="text" name="name" id="name" value="{{ $cast->name }}" required
                ...
                <div class="mb-6">
                    ...
                    <input type="date" name="birth_date" id="birth_date" value="{{ $cast->birth_date }}"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 p-2" />
                    @error('birth_date')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
